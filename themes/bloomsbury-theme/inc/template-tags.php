 ...
 */
  ...
  */
 function bloomsbury_comment_count() {
  ...
  */
 function bloomsbury_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && bloomsbury_categorized_blog() ) {
  ...
  * @return bool
  */
 function bloomsbury_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'bloomsbury_categories' ) ) ) {
 		...
 		set_transient( 'bloomsbury_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so bloomsbury_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so bloomsbury_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in bloomsbury_categorized_blog.
  */
 function bloomsbury_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'bloomsbury_categories' );
 }
 add_action( 'edit_category', 'bloomsbury_category_transient_flusher' );
 add_action( 'save_post',     'bloomsbury_category_transient_flusher' );
  ...
  */
 function bloomsbury_comment_list( $comment, $args, $depth ) {
  ...
  */
 function bloomsbury_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
